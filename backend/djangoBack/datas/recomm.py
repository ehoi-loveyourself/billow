...
def predict_table():
    ...
    # df_svd_preds = pd.DataFrame(svd_user_predicted_ratings, columns = user_list)
...
#     df_svd_preds.set_index(user_id, inplace=True)
...
#     # sorted_user_predictions = df_svd_preds.loc[df_svd_preds["user_id"]==user_id].sort_values(ascending=False)
...
#     recommendations = recommendations.sort_values(user_id, ascending = False).iloc[:num_recommendations, :]
